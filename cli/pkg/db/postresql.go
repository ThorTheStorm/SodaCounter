...
func TableAddContent(db *sql.DB, tableName string, title string, content string, author string) error {
	query := fmt.Sprintf(`
		INSERT INTO %s (title, content, author)
		VALUES ($1, $2, $3);
	`, tableName)
	_, err := db.Exec(query, title, content, author)
	if err != nil {
		return fmt.Errorf("failed to insert content into table %s: %v", tableName, err)
	}
	return nil
} // func TableAddContent

func TableGetContent(db *sql.DB, tableName string, id int) (*sql.Rows, error) {
	query := fmt.Sprintf(`
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get content from table %s: %v", tableName, err)
	}
	return response, nil
} // func TableGetContent

func TableUpdateContent(db *sql.DB, tableName string, id int, title string, content string, author string) error {
	query := fmt.Sprintf(`
		UPDATE %s
